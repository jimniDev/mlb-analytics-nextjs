      ...
      <div className="p-4 rounded-lg" style={{ backgroundColor: "#f9f9f9" }}>
        ...
        <p className="text-sm">
          ...
          <span className="font-bold">
            {" "}
            ${(teamData.avgPayroll / 1000000).toFixed(1)}M{" "}
          </span>
          per season, resulting in an average of
          <span className="font-bold">
            {" "}
            {teamData.avgWins.toFixed(1)} wins{" "}
          </span>
          per season.
        </p>
        ...
        <p className="text-sm mt-2">
          ...
          <span className="font-bold">
            {" "}
            ${(teamData.avgCostPerWin / 1000000).toFixed(2)}M{" "}
          </span>
          per win, which ranks
        ...
        <p className="text-sm">
          ...
          out of 4 seasons
          {teamData.worldSeriesWins > 0
            ? ` and won ${teamData.worldSeriesWins} World Series.`
            : "."}
        </p>

        <p className="text-sm mt-4 italic">{efficiencyAssessment}</p>
      </div>
    </div>
  );
};
